  ...
  return (
    <div className="flex justify-center h-screen  ">
      <div className=" bg-green-400 m-5  w-2/3 rounded-3xl">
        ...
        {tasks.map((todo) => {
          if(showfinished)
          return ((todo.iscompleted)&&(<div key={todo.id}  className={"mx-8 mt-4 px-4 font-bold font-mono bg-purple-400 rounded-xl h-7 flex items-center text-left".concat(todo.iscompleted ?" line-through": "")}><input type='checkbox' checked={todo.iscompleted} name={todo.id} className="mx-2" onChange={handlecheckboxchange}></input>
            
            <span>{todo.tasktext}</span>
            <div className="ml-auto">
            <button ref={editbtn} className="bg-yellow-400 p-1 h-1/2 rounded-lg" onClick={(e)=>handlechangeedit(e,todo.id)}><FaRegEdit /></button>
            <button className="bg-red-500 p-1 m-1 rounded-lg" onClick={(e)=>handlechangedelete(e,todo.id)}><FaRegTrashCan /></button>
            </div>
            </div>));
          else
          return ((!todo.iscompleted)&&(<div key={todo.id}  className={"mx-8 mt-2 px-4 font-bold font-mono bg-purple-400 rounded-xl h-7 flex items-center text-left".concat(todo.iscompleted ?" line-through": "")}><input type='checkbox' checked={todo.iscompleted} name={todo.id} className="mx-2" onChange={handlecheckboxchange}></input>
            <span>{todo.tasktext}</span>
            <div className="ml-auto">
            <button ref={editbtn} className="bg-yellow-400 h-1/2 p-1 rounded-lg" onClick={(e)=>handlechangeedit(e,todo.id)}><FaRegEdit /></button>
            <button className="bg-red-500 p-1 m-1 h-1/2 rounded-lg" onClick={(e)=>handlechangedelete(e,todo.id)}><FaRegTrashCan /></button>
            </div>
            </div>));
        })}
      </div>
